...
class RCModel(object):
    ...
    def evaluate(self, eval_batches, result_dir=None, result_prefix=None, save_full_info=False):
        ...
        for b_itx, batch in enumerate(eval_batches):
            ...
            # sample, and start_prob , end_prob for each sample
            for sample, start_prob, end_prob in zip(batch['raw_data'], start_probs, end_probs):
                best_answer,best_p_idx,best_para_idx = self.find_best_answer(sample, start_prob, end_prob, padded_p_len)
                if save_full_info:
    ...
    def find_best_answer(self, sample, start_prob, end_prob, padded_p_len):
        ...
        """
        best_p_idx,best_para_idx, best_span, best_score = None,None, None, 0
        for p_idx, passage in enumerate(sample['passages']):
            ...
            answer_span, score = self.find_best_answer_for_passage(
                ...
                passage_len)
            if score > best_score:
                # check if answer only include punctuation
                answer_check = ''.join(
                passage['passage_tokens'][answer_span[0]: answer_span[1] + 1])
                chinese_punctuation = ['。','，','；','：','？','《','》','!','"','、','【','】']
                answer_check = answer_check.strip()
                if len([c for c in answer_check if c not in punctuation and c not in chinese_punctuation]):
                    best_score = score
                    best_p_idx = p_idx
                    best_span = answer_span
                    best_para_idx = passage['para_idx']
        if best_p_idx is None or best_span is None:
            best_answer = ''.join(sample['passages'][0]['passage_tokens'])
            best_para_idx = sample['passages'][0]['para_idx']
            best_p_idx = 0
        else:
            best_answer = ''.join(
                sample['passages'][best_p_idx]['passage_tokens'][best_span[0]: best_span[1] + 1])
        return best_answer,best_p_idx,best_para_idx  # return best_p_idx to indicate which doc choose
